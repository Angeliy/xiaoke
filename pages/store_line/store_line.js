...
Page({
  ...
  getwayprocet: function () {
    ...
    comm.requestPost("/service/employer/viewOrders.do", data, function (res) {
      if (res.data.result == 200 && typeof res.data.data.list != "undefined") {
        var list = res.data.data.list;
        for (var i in list) {
          list[i].orderTime = comm.js_date_time(list[i].orderTime);
          var projectStr = "";
          for (var k in list[i].projects) {
            projectStr += list[i].projects[k].projectName + " ";
          }
          list[i].projectStr = projectStr;
  ...
  openConfirm: function (e) {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          comm.requestPost("/service/employer/confirmOrder.do", data, function (res) {
